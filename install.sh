...
# NetDeck Installation Script
# Automatically downloads and installs the latest NetDeck release from GitHub

set -euo pipefail  # Exit on error, undefined vars, and pipe failures

# Configuration
readonly PACKAGE="NetDeck"
...
download_and_install() {
    ...
    if ! sudo rsync -av "${temp_dir}/${PACKAGE}/" "$PLUGIN_DIR" --delete; then
        # Try without PACKAGE subdirectory (flat archive structure)
        log_debug "Trying flat archive structure..." >&2
        if ! sudo rsync -av "${temp_dir}/" "$PLUGIN_DIR" --delete --exclude="*.zip"; then
